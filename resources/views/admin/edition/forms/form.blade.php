            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-8">
                        ...
                        <div class="form-group row validated">
                            <label class="col-form-label col-lg-3 col-sm-12 text-lg-left">Location </label>
                            <div class="col-lg-9 col-md-9 col-sm-12">
                                <input type="location" name="location" value="{{ old('location', $row->location ?? '') }}" class="form-control" placeholder="Enter Location" required />
                                @error('location')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="form-group row validated">
                            ...
                            <div class="col-lg-9 col-md-9 col-sm-12">
                                <textarea name="description" class="form-control" placeholder="Enter Description" >{{ old('description', $row->description ?? '') }}</textarea>
                                @error('description')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="form-group row validated">
                            ...
                            <div class="col-lg-9 col-md-9 col-sm-12">
                                <div class="image-input image-input-outline" id="editionImage" style="background-image: url({{ asset('assets/backend/media/users/blank.png') }})">
                                    @if(isset($row->image) && !empty($row->image))
                                        <div class="image-input-wrapper" style="background-image: url({{ asset('uploads/editions/thumbnails/250/'.$row->image) }})"></div>
                                    @else
...
<script type="text/javascript">
    var editionImage = new KTImageInput('editionImage');

    if(typeof editionImage != 'underfined') {

        editionImage.on('cancel', function(imageInput) {
            __sweetAlert('Image successfully canceled!', 'success');
        });

        editionImage.on('change', function(imageInput) {
            console.log("editionImage this==>", this);
        });

        editionImage.on('remove', function(imageInput) {
